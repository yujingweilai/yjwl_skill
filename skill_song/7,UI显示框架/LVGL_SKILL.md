...
当 `app_ui.h` 已定义 `app_ui_shared_data_t`，标签类型、RTC 日期、元素名字、详情和标签业务日期等跨页面数据必须通过下面的公共接口链路传递。不得让外围组件直接修改 `app_ui.c` 内部缓存，也不得把外围任务的临时地址长期保存到 `user_data`。

```text
外围业务任务
    ↓ 只调用 app_arch.h 中的线程安全接口
app_arch_update_ui_shared_data(source, data)
    ↓ 检查参数并获取 s_ui_api_mutex
app_ui_update_shared_data(data)
    ↓ 完整复制到 app_ui.c 的静态长期缓存
static app_ui_shared_data_t s_ui_shared_data
    ↓ page_data.user_data 只指向 &s_ui_shared_data
当前页面 draw(display, ctx)
    ↓ 转换为 const app_ui_shared_data_t * 后只读使用
LVGL 控件更新
```

#### 接口职责与声明规范

1. `app_ui.h` 负责声明公共结构体和 UI 内部更新接口；`app_ui.c` 负责定义静态缓存和实现深拷贝，禁止在头文件中定义缓存实例。
2. `app_arch.h` 负责声明外围组件可调用的线程安全接口；`app_arch.c` 必须复用现有 `s_ui_api_mutex`，禁止另外创建平行 UI 数据通道。
3. 如果工程尚无对应接口，优先按现有命名风格补充以下声明；如果已有等价接口，必须复用已有接口，不得重复造函数。

```c
/**
 * @brief 更新 UI 公共业务数据
 * @param data 公共业务数据指针，函数内部会完整复制其内容
 * @return ESP_OK 表示更新成功，其他值表示参数无效或更新失败
 * @note 作用：把调用方数据复制到 app_ui 内部长期缓存，不保存调用方指针
 */
esp_err_t app_ui_update_shared_data(const app_ui_shared_data_t *data);

/**
 * @brief 从外围业务任务更新 UI 公共业务数据
 * @param source UI 请求来源，用于日志记录和调用来源检查
 * @param data 公共业务数据指针，接口返回后调用方可以释放或复用原数据
 * @return ESP_OK 表示更新成功，其他值表示参数无效、锁失败或内部更新失败
 * @note 作用：通过 app_arch 互斥锁串行提交数据，再交给 app_ui 完整复制
 */
esp_err_t app_arch_update_ui_shared_data(app_arch_ui_source_t source,
                                         const app_ui_shared_data_t *data);
```

> 以上函数名是本项目新增接口的推荐名称，不代表函数已经存在。修改代码前必须先搜索声明和实现；只有用户要求实现公共数据传输时才能新增，并同步修改 `.h` 声明、`.c` 实现和组件调用处。

#### 缓存、读取和刷新规则

1. `app_ui.c` 使用 `static app_ui_shared_data_t s_ui_shared_data` 长期持有数据；页面切换或删除旧页面 LVGL 对象时不得清空该缓存。
2. `app_ui_update_shared_data()` 必须检查 `data != NULL`，然后按值完整复制结构体；结构体内字符串必须使用定长数组，禁止增加依赖调用方生命周期的裸字符串指针。
3. 当 `user_data` 用于访问公共业务数据时，只能绑定 `&s_ui_shared_data` 等 `app_ui` 内部长期有效地址，禁止保存公共接口传入参数 `data` 的地址；仅用于一次临时页面交换时可以绑定其他临时数据，但该数据必须保持到页面停止使用并在退出后清除引用。
4. 页面读取时必须使用 `const app_ui_shared_data_t *shared_data`，只允许读，禁止页面直接反向修改公共缓存。
5. 公共数据变化时，若当前页面需要显示该数据，则设置既有刷新标志并走 `app_ui_update_display()`；若当前页面不使用该数据，只更新缓存，不强制切页。
6. 同一公共结构体多数成员同时变化时使用完整更新接口；只有高频单字段更新确有必要时，才沿用工程已有 patch 模式增加字段掩码，禁止为每个字段随意创建一个函数。
7. 读取接口优先采用“复制到调用方输出参数”的方式，并由 `app_arch` 互斥保护；不得把可写的内部缓存指针暴露给外围任务。页面内部只读访问可通过 `ctx->page_data.user_data` 完成。
8. **长期使用数据和页面交互数据一般必须走公共接口。** 标签类型、RTC 日期、元素名字、详情、业务日期、用户选择及一个页面修改后还要交给其他页面的数据，必须复制到 `app_ui.c` 的静态长期缓存，不能依赖旧页面对象或临时指针。
9. **临时数据才允许直接使用 `app_ui_page_data_t.user_data`。** 该数据应只服务于一次临时页面交换，页面退出后允许消失，也不需要被其他页面或外围组件继续读取。
10. 临时 `user_data` 的有效期必须覆盖页面完整使用期间；禁止指向函数局部变量、任务栈变量、临时字符串或即将释放的动态内存。页面退出后必须清除引用并停止读取旧指针。
11. 无法判断数据是否需要长期保留时，默认使用公共接口和内部深拷贝，不能默认使用 `user_data`。
12. `app_arch` 获取 `s_ui_api_mutex` 后不得直接调用 LVGL；真正的 LVGL 操作仍只能在渲染链路持有 `lvgl_port_lock()` 时进行。
13. ISR 禁止调用这些接口；ISR 必须先使用 `xQueueSendFromISR()` 或任务通知交给普通任务。
14. 新增函数必须写清楚函数名、参数、返回值和作用的中文注释；参数错误、锁失败、更新成功等关键日志必须使用英文 `ESP_LOGE/W/I`，但禁止高频刷屏。

#### 数据接口选择速查

| 数据场景 | 应使用的接口 | 生命周期要求 |
|---|---|---|
| 多个页面共同使用 | `app_arch` 公共业务数据接口 | 保存到 `app_ui` 静态长期缓存 |
| 页面之间继续交互 | `app_arch` 公共业务数据接口 | 旧页面删除后数据仍有效 |
| RTC、标签信息、用户配置 | `app_arch` 公共业务数据接口 | 按值完整复制，不保存调用方地址 |
| 一次临时页面交换 | `app_ui_page_data_t.user_data` | 数据至少活到页面停止使用 |
| 页面退出后允许消失 | `app_ui_page_data_t.user_data` | 退出后清除引用，不再访问 |

大白话：长期数据和页面之间还要继续传的数据放进公共接口，就像存进长期保险箱；只在当前临时页面用一下、关页后可以丢掉的数据才使用 `user_data`。`user_data` 只是借用一个地址，不会复制内容。

#### 公共数据接口复核项

- 已确认没有把局部变量、函数栈结构体或临时字符串地址保存到 `user_data`。
- 已确认 `app_ui_shared_data_t` 是完整复制，不是只复制外部指针。
- 已确认页面删除只释放 LVGL 对象，不会清除公共业务缓存。
- 已确认目标页面未显示时只保存数据，不会擅自切换页面。
- 已确认外围组件只包含 `app_arch.h`，没有直接调用 `app_ui_update_shared_data()`。
- 已确认新增函数声明与实现完全一致，没有引入未知变量、未知函数或重复接口。
- 已确认没有使用 `goto`，日志为英文，所有新增函数均有完整中文注释。

大白话：外围任务把一整份数据交给 `app_arch`，`app_arch` 排队后交给 `app_ui`，`app_ui` 再把内容抄进自己的长期保险箱。页面切走只会删画面控件，不会删保险箱里的数据；以后再进入页面，直接从保险箱取最新数据重新显示。

### 温湿度数据约定

温度和湿度应作为同一个业务结构体关联传递，建议使用定点整数（例如 `temperature_x10=253` 表示 `25.3℃`）。持续刷新、跨页面保留或多个组件共用时，必须通过公共接口复制到 `app_ui` 长期缓存；只有页面退出后允许丢失的一次性显示数据，才通过 `ctx->page_data.user_data` 临时读取，并保证该数据在页面停止使用前一直有效。
